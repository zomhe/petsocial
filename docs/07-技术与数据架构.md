...
| 数据库 | PostgreSQL | 事务、约束、JSONB 与地理扩展能力 |
...
- 客户端坐标经服务端转换为网格或距离档；列表响应不含原始坐标。
- 公共集合点由平台维护，邀约只保存 place_id 和当时名称/区域快照。
- 附近查询先按网格粗筛，再计算距离并返回模糊档位；拉黑、隐身、社区规则最后过滤。
...
- `walk_invitations`：主状态、条件 JSONB、容量、公共地点快照、版本。
...
- 所有外部 ID 使用 UUID/不可枚举 ID；内部序列不直接暴露。
...
- 软删除表的唯一约束使用部分索引，只覆盖 `deleted_at is null`。
